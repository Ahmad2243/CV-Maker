...
/*childContainer1----------------------------------------------------------------*/  
  ...
  const [inputEmail, setInputEmail] = useState<string>('ahmadrabbani@gmail.com');
  const [inputLocation, setInputLocation] = useState<string>('Karachi,Pakistan.');
...
/*childContainer2----------------------------------------------------------------*/
  ...
  const [inputValue, setInputValue] = useState<string>('Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere minus, saepe iste, nemo dolores necessitatibus perspiciatis velit esse distinctio consequuntur recusandae asperiores sunt veritatis eum quibusdam culpa vero facilis eos?');
...
/*childContainer3-----------------------------------------------------------------------*/
const [expandEducationAccordion, setExpandEducationAccordion] = useState<boolean>(false);
  const [showEducationModal, setShowEducationModal] = useState<boolean>(false);
  const [inputInstitutes, setInputInstitutes] = useState<string>('Sindh Madrassa Tul Islam University');
  const [inputDegree, setInputDegree] = useState<string>('BS Information Technology');
...
/*----------------------------------------------------------------*/
/*childContainer4----------------------------------------------------------------*/
const [expandProfessionalAccordion, setExpandProfessionalAccordion] = useState(false);
      ...
      <div onClick={handleExpandEducation} className="childContainer3">
        ...
        {showEducationModal && (
          <Education
            ...
            onDustbinClick={() => handleDustbinClick(submittedValues.length - 1)}
          />
        )}
      </div>
            ...
            <>
              {proSubmittedValues.map((value, proindex) => (
                <div
                  key={proindex}
                  id="submitted-value"
                  onClick={(event) => handleProSubmittedValueClick(proindex, event)}
                >
              ...
              {submittedSkills.map((skill, skillsIndex) => (
                ...
                >
                  <input
                    ...
                    type="text"
                    value={skill.skills}
                    onChange={(event) => handleSkillsInputChange(event, skillsIndex)}
                  ...
                  <button
                    type="button"
                    id="dustbin"
                    onClick={(event) => handleSkillsDustbinClick(skillsIndex, event)}
                  >
                    🗑️
                  </button>
                </div>
